...
|------|--------|------------|
| General: Unsupervised clustering of ALMA protoplanetary disk images | Convolutional Autoencoder + K-means + UMAP | Silhouette = 0.474, MSE = 0.00687 |
...
- **Architecture**: Convolutional Autoencoder (1,368,305 params), two-phase training (MSE → MSE+SSIM)
- **Augmentation**: Random 0–360° rotation + horizontal/vertical flips (orientation invariance)
- **Clustering**: K-means k=6 in 64-dim latent space (physically motivated by planet-count ranges)
- **Reconstruction MSE**: 0.00687
- **Silhouette score**: 0.474 (k=6); mathematically optimal k=3 gives 0.626
